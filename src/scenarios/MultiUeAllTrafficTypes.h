...
static void MultiUeAllTrafficTypes (int scheduler, int nbUE, int speed, double maxDelay, int simulationTime, int seed, int cellSize)
{
    ...
    for(int idUE=2; idUE<2+nbUE; idUE++)
    {
      ...
      // create application
      BEApplication[beApplicationCounter].SetSource (gateway);
      BEApplication[beApplicationCounter].SetDestination (ue);
      BEApplication[beApplicationCounter].SetApplicationID (applicationID);
      BEApplication[beApplicationCounter].SetStartTime(startTime);
      BEApplication[beApplicationCounter].SetStopTime(startTime + durationTime);

      // create qos parameters
      QoSParameters *qosParameters = new QoSParameters ();
      BEApplication[beApplicationCounter].SetQoSParameters (qosParameters);
      ...
      BEApplication[beApplicationCounter].SetClassifierParameters (cp);

      std::cout << "CREATED NON-RT APPLICATION, ID " << applicationID << std::endl;

      //update counter
      destinationPort++;
      applicationID++;
      beApplicationCounter++;
