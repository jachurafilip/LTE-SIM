...
for ue=1:length(users)
    for s=1:length(sched)
        dat=load(strcat('../OUT/AllTraffic/',test,'_SCHED_',num2str(sched(s)),'_UE_',num2str(users(ue)), '_V_',num2str(speed(v)),'_D_',num2str(delay(d))));
        mat(s,ue)=mean(dat);
    end
    end
    t=strcat('AllTraffic/',test,'_SPEED_',num2str(speed(v)),'_UEs_',num2str(users(ue)),'_D_',num2str(delay(d)));
